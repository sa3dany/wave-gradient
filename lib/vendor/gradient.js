 ...
 * Amplitude:    Gradient.amp = 0 Colors:       Gradient.sectionColors
 * (if you change colors, use normalizeColor(#hexValue)) before you
 * assign it.
 *
...
export class Gradient {
  ...
  animate(e) {
    if (!this.shouldSkipFrame(e)) {
      if (
        ((this.t += Math.min(e - this.last, 1e3 / 15)), (this.last = e), false)
      ) {
        let e = 160;
    ...
    this.conf.playing && requestAnimationFrame(this.animate.bind(this));
  }
  ...
  play() {
    requestAnimationFrame(this.animate.bind(this)), (this.conf.playing = true);
  }
