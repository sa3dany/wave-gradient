...
export const snoise_glsl = `vec3 o(vec3 f){return f-floor(f*(1./289.))*289.;}vec4 o(vec4 f){return f-floor(f*(1./289.))*289.;}vec4 e(vec4 f){return o((f*34.+1.)*f);}vec4 i(vec4 z){return 1.79284291400159-.85373472095314*z;}float v(vec3 w){const vec2 m=vec2(1./6.,1./3.);const vec4 p=vec4(0.,.5,1.,2.);vec3 s=floor(w+dot(w,m.yyy)),C=w-s+dot(s,m.xxx),S=step(C.yzx,C.xyz),P=1.-S,F=min(S.xyz,P.zxy),R=max(S.xyz,P.zxy),L=C-F+m.xxx,b=C-R+m.yyy,W=C-p.yyy;s=o(s);vec4 g=e(e(e(s.z+vec4(0.,F.z,R.z,1.))+s.y+vec4(0.,F.y,R.y,1.))+s.x+vec4(0.,F.x,R.x,1.));vec3 G=.142857142857*p.wyz-p.xzx;vec4 q=g-49.*floor(g*G.z*G.z),h=floor(q*G.z),O=floor(q-7.*h),f=h*G.x+G.yyyy,B=O*G.x+G.yyyy,A=1.-abs(f)-abs(B),E=vec4(f.xy,B.xy),k=vec4(f.zw,B.zw),Z=floor(E)*2.+1.,Y=floor(k)*2.+1.,X=-step(A,vec4(0.)),V=E.xzyw+Z.xzyw*X.xxyy,U=k.xzyw+Y.xzyw*X.zzww;vec3 T=vec3(V.xy,A.x),Q=vec3(V.zw,A.y),N=vec3(U.xy,A.z),M=vec3(U.zw,A.w);vec4 K=i(vec4(dot(T,T),dot(Q,Q),dot(N,N),dot(M,M)));T*=K.x;Q*=K.y;N*=K.z;M*=K.w;vec4 J=max(.6-vec4(dot(C,C),dot(L,L),dot(b,b),dot(W,W)),0.);J=J*J;return 42.*dot(J*J,vec4(dot(T,C),dot(Q,L),dot(N,b),dot(M,W)));}
`;
